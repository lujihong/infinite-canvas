...
import { createVideoGenerationTask, formatPlayableVideoUrl, pollVideoGenerationTaskStatus, VIDEO_POLL_INTERVAL_MS, type VideoResponse } from "@/services/api/video";
...
import { sortStoryboardVideoNodes } from "../utils/canvas-storyboard-sort";
...
import { DEFAULT_CANVAS_AGENT_PANEL, DEFAULT_CANVAS_SIDE_PANEL, flushPendingCanvasProjectSaves, normalizeCanvasProject, useCanvasStore } from "../stores/use-canvas-store";
...
function InfiniteCanvasPage({ projectId }: { projectId: string }) {
    ...
    const resolvedAgentConfig = useMemo<CanvasAgentConfig>(
        () =>
            agentConfig ? (Object.assign({ textApiMode: "chat" as const, autoGenerateMedia: true }, agentConfig) as CanvasAgentConfig) : {
                textApiMode: "chat",
    ...
    useEffect(() => {
        ...
        const restore = async () => {
            ...
            setAgentConfig(project.agentConfig ? (Object.assign({ textApiMode: "chat" as const, autoGenerateMedia: true }, project.agentConfig) as CanvasAgentConfig) : null);
    ...
    useEffect(() => {
        ...
        if (!user) {
            ...
            setProjectLoaded(false);
            router.replace(`/login?redirect=${encodeURIComponent(`/canvas/${projectId}`)}`);
        }
    }, [isUserReady, user, projectId, router]);

    useEffect(() => {
        if (!projectLoaded || historyPausedRef.current || !user) return;
        updateProject(projectId, { nodes, connections, chatSessions, activeChatId, agentConfig, backgroundMode, showImageInfo });
    }, [activeChatId, agentConfig, backgroundMode, chatSessions, connections, nodes, projectId, projectLoaded, showImageInfo, updateProject, user]);

    // 页面刷新、离线或切换前强制急刷保存，保证连线与节点100%不丢失
    useEffect(() => {
        const handleFlush = () => {
    ...
    useEffect(() => {
        ...
        const pollCanvasTasks = () => {
            const isTaskEnded = (status?: string, mediaUrl?: string) => {
                const s = String(status || "").toLowerCase();
                return ["completed", "complete", "done", "succeeded", "success", "failed", "fail", "error", "cancelled", "canceled"].includes(s) || Boolean(mediaUrl);
            };
            ...
            videoTargets.forEach((node) => {
                ...
                void pollVideoGenerationTaskStatus(generationConfig, canvasVideoTaskFromMetadata(node.metadata))
                    .then((task) => {
                        ...
                        if (isTaskEnded(task.status, task.video_url || task.url)) {
            ...
            imageTargets.forEach((node) => {
                ...
                void pollCanvasImageTaskStatus(node.metadata.imageTaskId)
                    .then((task) => {
                        ...
                        if (isTaskEnded(task.status, task.image_url || task.url)) {
                            void useWalletStore.getState().triggerWalletSync();
                        }
                    })
                    .catch(() => undefined)
            ...
            audioTargets.forEach((node) => {
                ...
                void pollCanvasAudioTaskStatus(node.metadata.audioTaskId)
                    .then((task) => {
                        ...
                        if (isTaskEnded(task.status, task.audio_url || task.url)) {
                            void useWalletStore.getState().triggerWalletSync();
                        }
                    })
                    .catch(() => undefined)
    ...
    const createAndOpenProject = useCallback(() => {
        const id = createProject(`新项目 ${useCanvasStore.getState().projects.length + 1}`);
    ...
    const downloadNodeImage = useCallback(async (node: CanvasNodeData) => {
        ...
        // 如果是受保护的视频 content 地址（需要携带 Token 授权下载）
        if (node.type === CanvasNodeType.Video && /\/videos\/[^/]+\/content(?:[?#]|$)/i.test(contentUrl)) {
            const hideLoading = message.loading("正在准备视频下载...", 0);
            try {
                const taskIdMatch = contentUrl.match(/\/videos\/([^/?#]+)\/content/i);
                const taskId = (taskIdMatch ? taskIdMatch[1] : "") || node.metadata?.videoTaskId || "";
                const videoModel = node.metadata?.model || effectiveConfig.videoModel || effectiveConfig.model || "";
                const token = useUserStore.getState().token;
                const headers: Record<string, string> = token ? { Authorization: `Bearer ${token}` } : {};
                const fetchUrl = taskId
                    ? `/api/v1/videos/${encodeURIComponent(taskId)}/content?model=${encodeURIComponent(videoModel)}`
                    : contentUrl;
                ...
                message.error(err instanceof Error ? err.message : "视频下载失败");
            } finally {
                hideLoading();
            }
            return;
        ...
        saveAs(contentUrl, fileName);
    }, [effectiveConfig.model, effectiveConfig.videoModel, message]);
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            ...
            if (node.type === CanvasNodeType.Video) {
                ...
                addAsset({ kind: "video", title: node.metadata?.prompt?.slice(0, 24) || "画布视频", coverUrl: "", tags: [], source: "Canvas", data: { url: node.metadata.content, storageKey: node.metadata.aiccUri ? undefined : node.metadata.storageKey, aiccUri: node.metadata.aiccUri, aiccChannelId: node.metadata.aiccChannelId, width: node.metadata.naturalWidth || node.width, height: node.metadata.naturalHeight || node.height, bytes: node.metadata.bytes || 0, mimeType: node.metadata.mimeType || "video/mp4" }, metadata: { source: "canvas", nodeId: node.id, prompt: node.metadata?.prompt } });
                message.success("已加入我的素材");
                return;
            }
            if (node.type === CanvasNodeType.Audio) {
                if (!node.metadata?.content) return message.error("没有可保存的音频");
                addAsset({ kind: "audio", title: node.metadata.prompt?.slice(0, 24) || "画布音频", coverUrl: "", tags: [], source: "Canvas", data: { url: node.metadata.content, storageKey: node.metadata.aiccUri ? undefined : node.metadata.storageKey, aiccUri: node.metadata.aiccUri, aiccChannelId: node.metadata.aiccChannelId, bytes: node.metadata.bytes, mimeType: node.metadata.mimeType || "audio/mpeg", durationMs: node.metadata.durationMs }, metadata: { source: "canvas", nodeId: node.id, prompt: node.metadata.prompt } });
                message.success("已加入我的素材");
                return;
            }
            if (!node.metadata?.content) return message.error("没有可保存的图片");
            try {
                const stored = !node.metadata.aiccUri && !node.metadata.storageKey && node.metadata.content.startsWith("blob:") ? await uploadImage(node.metadata.content, { localOnly: true }) : null;
                const imageUrl = stored?.url || node.metadata.content;
                const storageKey = node.metadata.aiccUri ? undefined : stored?.storageKey || node.metadata.storageKey;
            ...
            if (target?.nodeId) {
                ...
                try {
                    if (isAudioFile(file)) {
                        const audio = await uploadMediaFile(file, "audio");
                        const spec = NODE_DEFAULT_SIZE[CanvasNodeType.Audio];
                        setNodes((prev) => prev.map((node) => (node.id === target.nodeId ? { ...node, type: CanvasNodeType.Audio, title: file.name, position: { x: node.position.x + node.width / 2 - spec.width / 2, y: node.position.y + node.height / 2 - spec.height / 2 }, width: spec.width, height: spec.height, metadata: { ...node.metadata, ...REPLACED_MEDIA_METADATA, ...audioMetadata(audio) } } : node)));
                        setSelectedNodeIds(new Set([target.nodeId]));
                        setSelectedConnectionId(null);
                    } else if (file.type.startsWith("video/")) {
                        const video = await uploadMediaFile(file, "video");
                        const nextSize = fitNodeSize(video.width || 1280, video.height || 720, VIDEO_NODE_MAX_WIDTH, VIDEO_NODE_MAX_HEIGHT);
                        setNodes((prev) => prev.map((node) => (node.id === target.nodeId ? { ...node, type: CanvasNodeType.Video, title: file.name, position: { x: node.position.x + node.width / 2 - nextSize.width / 2, y: node.position.y + node.height / 2 - nextSize.height / 2 }, width: nextSize.width, height: nextSize.height, metadata: { ...node.metadata, ...REPLACED_MEDIA_METADATA, ...videoMetadata(video) } } : node)));
                        setSelectedNodeIds(new Set([target.nodeId]));
                        setSelectedConnectionId(null);
    ...
    function insertAssetAt(payload: InsertAssetPayload, position?: Position, nodeId?: string) {
        ...
        if (payload.aiccUri) {
            ...
            setNodes(prev => [...prev, {id, type, title: payload.title, position: {x:center.x-spec.width/2,y:center.y-spec.height/2}, width:spec.width,height:spec.height, metadata:{content:preview,aiccUri:payload.aiccUri,aiccChannelId:payload.aiccChannelId,status:NODE_STATUS_SUCCESS,mimeType:payload.mimeType}}]);
            setSelectedNodeIds(new Set([id]));
            setSelectedConnectionId(null);
            return;
        }
        ...
        async (nodeId: string, mode: CanvasNodeGenerationMode, prompt: string) => {
            ...
            try {
                ...
                if (mode === "video") {
                    ...
                    const effectiveVideoConfig = videoGenerationConfig;
        ...
        async (action: CanvasAgentAction, messageReferenceNodeIds: string[]): Promise<CanvasAgentToolResult> => {
            ...
            try {
                ...
                if (action.name === "generate_image" || action.name === "edit_image" || action.name === "generate_video" || action.name === "generate_audio") {
                    ...
                    const explicitModel = stringValue("model");
                    const upstreamVideoModel = mode === "video" ? sourceNodes.find((node) => node.type === CanvasNodeType.Video)?.metadata?.model : undefined;
                    const upstreamImageModel = mode === "image" ? sourceNodes.find((node) => isCanvasImageNodeType(node.type))?.metadata?.model : undefined;
                    const targetOverrideModel = explicitModel || upstreamVideoModel || upstreamImageModel;

                    const baseGenConfig = buildGenerationConfig(agentEffectiveConfig, undefined, mode);
                    const generationConfig = {
                        ...baseGenConfig,
                        ...(targetOverrideModel ? {
                            model: targetOverrideModel,
                            ...(mode === "video" ? { videoModel: targetOverrideModel } : {}),
                            ...(mode === "image" ? { imageModel: targetOverrideModel } : {}),
                        } : {}),
                    };
                ...
                if (action.name === "create_video_montage") {
                    const requestedIds = stringValues("videoNodeIds");
                    let validVideoNodes: CanvasNodeData[] = [];

                    if (requestedIds.length > 0) {
                        // 若显式传入 ID 列表，严格尊重用户/模型给定的叙事编排顺序
                        validVideoNodes = requestedIds
                            .map((id) => getNode(id))
                            .filter((n): n is CanvasNodeData => Boolean(n && n.type === CanvasNodeType.Video && n.metadata?.content));
                    } else {
                        // 框选模式：综合连线箭头流向、分镜编号与空间故事板行容差阅读流向进行智能时序编排
                        const rawNodes = Array.from(selectedNodeIdsRef.current)
                            .map((id) => getNode(id))
                            .filter((n): n is CanvasNodeData => Boolean(n && n.type === CanvasNodeType.Video && n.metadata?.content));
                        validVideoNodes = sortStoryboardVideoNodes(rawNodes, connectionsRef.current);
                    }

                    if (!validVideoNodes.length) {
                        return {
                            ok: false,
                            code: "no_valid_video_nodes",
                            message: "未在画布中找到已生成完成的有效视频镜头，请确认指定或选中的节点已生成视频内容",
                        };
                    }

                    const title = stringValue("title") || `${currentProject?.title || "画布"} 视频剪辑成片`;
                    const aspectRatio = stringValue("aspectRatio");
                    const batchParam = validVideoNodes.map((n) => n.id).join(",");
                    let editorUrl = `/editor?projectId=${encodeURIComponent(projectId)}&batchNodes=${batchParam}&title=${encodeURIComponent(title)}`;
                    if (aspectRatio && ["16:9", "9:16", "1:1", "4:3"].includes(aspectRatio)) {
                        editorUrl += `&aspectRatio=${encodeURIComponent(aspectRatio)}`;
                    }

                    // 自动选中高亮这些视频分镜
                    setSelectedNodeIds(new Set(validVideoNodes.map((n) => n.id)));

                    // 在新标签页中打开视频剪辑台（防弹窗拦截探测）
                    ...
                    const openNote = windowOpened
                        ? "已自动在新窗口打开剪辑台"
                        : "若浏览器拦截了新标签页弹窗，可直接点击剪辑台链接进入";

                    return {
                        ok: true,
                        message: `已为您自动将 ${validVideoNodes.length} 个视频镜头按时间顺序编排进多轨视频剪辑台（${openNote}），已准备就绪供您剪辑、调速调音、加对白字幕与极速导出。`,
                        editorUrl,
                        windowOpened,
                        aspectRatio: aspectRatio || "16:9",
                        clipCount: validVideoNodes.length,
                        clips: validVideoNodes.map((n) => ({ id: n.id, title: n.title || "视频分镜" })),
                    };
                }
    ...
    const handleRetryNode = useCallback(
        async (node: CanvasNodeData) => {
            ...
            // 必须 100% 尊重当前节点自身选择的模型，用户选哪个就用哪个，绝不擅自篡改或回退
            const currentNodeModel = node.metadata?.model?.trim();
            const generationConfig = {
                ...baseConfig,
                ...(currentNodeModel ? {
                    model: currentNodeModel,
                    ...(node.type === CanvasNodeType.Video ? { videoModel: currentNodeModel } : {}),
                    ...(isCanvasImageNodeType(node.type) ? { imageModel: currentNodeModel } : {}),
                    ...(node.type === CanvasNodeType.Audio ? { audioModel: currentNodeModel } : {}),
                    ...(node.type === CanvasNodeType.Text ? { textModel: currentNodeModel } : {}),
                } : {}),
            };
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            for (const [index, asset] of request.assets.entries()) {
                ...
                if (payload.kind === "image" && payload.storageKey && !payload.aiccUri) {
                    const dataUrl = await resolveImageUrl(payload.storageKey, payload.dataUrl);
                    payload = { ...payload, dataUrl };
                } else if ((payload.kind === "video" || payload.kind === "audio") && payload.storageKey && !payload.aiccUri) {
    ...
    const handleAssetInsert = useCallback(
        (payload: InsertAssetPayload) => {
            ...
            if ((replacingNodeId || referenceTargetNodeId) && payload.kind === "text") {
                message.warning("请选择图片、视频或音频素材，文本不能用于替换媒体或添加媒体参考");
                return;
            }
            ...
            // 1. 替换现有节点逻辑
            if (replacingNodeId && payload.kind !== "text") {
                const targetNode = nodesRef.current.find((n) => n.id === replacingNodeId);
                if (!targetNode) {
                    message.warning("目标节点已不存在，请重新选择替换目标");
                    return;
                }
                if (isPanoramaNodeType(targetNode.type) && payload.kind !== "image") {
                    message.warning("全景图节点仅支持选择图片");
                    return;
                }
                replacingNodeIdRef.current = null;
                referenceTargetNodeIdRef.current = null;
                ...
                const nextPreview = payload.kind === "image" ? payload.dataUrl : payload.url;
                    ...
                    prev.map((node) => {
                        ...
                        return {
                            ...
                            metadata: {
                                ...
                                ...REPLACED_MEDIA_METADATA,
                                content: nextPreview,
                                storageKey: aiccUri ? undefined : payload.storageKey,
                                aiccUri,
                                aiccChannelId: "aiccChannelId" in payload ? payload.aiccChannelId : undefined,
                                ...
                                naturalWidth: "width" in payload ? payload.width : undefined,
                                naturalHeight: "height" in payload ? payload.height : undefined,
                            },
                        };
                    }),
                );
                message.success(aiccUri ? "已替换为移动云素材" : "已成功替换素材");
                return;
            }

            // 2. 作为参考内容添加到目标节点（自动在左侧新建素材节点并连线）
            if (referenceTargetNodeId && payload.kind !== "text") {
                const targetNode = nodesRef.current.find((n) => n.id === referenceTargetNodeId);
                if (!targetNode) {
                    message.warning("目标节点已不存在，请重新选择参考目标");
                    return;
                }
                replacingNodeIdRef.current = null;
                referenceTargetNodeIdRef.current = null;
                ...
                const newAssetNode: CanvasNodeData = {
                    id: newAssetId,
                    type,
                    title: payload.title || (aiccUri ? "移动云素材" : "参考素材"),
                    position: newPos,
                    width: spec.width,
                    height: spec.height,
                    metadata: {
                ...
                message.success(aiccUri ? "已添加移动云素材并关联为参考内容" : "已添加参考素材并自动关联连线");
                return;
            }
    ...
    const handleRepairAiccReference = useCallback((node: CanvasNodeData) => {
        const currentNodes = nodesRef.current;
        const incoming = (id: string) => currentNodes.filter((item) => connectionsRef.current.some((edge) => edge.toNodeId === id && edge.fromNodeId === item.id));
        const batchRoot = currentNodes.find((item) => item.id === node.metadata?.batchRootId);
        const result = batchRoot || node;
        const parents = incoming(result.id);
        const sources = parents.filter((item) => item.type === CanvasNodeType.Config);
        // 只有唯一配置源才可定位；多源时保留原图，交由用户明确选择参考。
        const source = result.type === CanvasNodeType.Config ? result : sources.length === 1 ? sources[0] : result;
        const inputs = (source.id === result.id ? parents : incoming(source.id)).filter((item) =>
            (isCanvasImageNodeType(item.type) || item.type === CanvasNodeType.Video || item.type === CanvasNodeType.Audio) &&
            item.metadata?.status !== NODE_STATUS_ERROR && Boolean(item.metadata?.content || item.metadata?.aiccUri),
        );
        if (sources.length <= 1 && inputs.length === 1) {
            focusNode(inputs[0].id);
            setDialogNodeId(inputs[0].id);
            handleReplaceMedia(inputs[0], "aicc");
            message.info("请选择移动云素材修复该参考输入；失败结果与原始错误会保留，请确认后手动生成");
            return;
        }
        focusNode(source.id);
        setDialogNodeId(source.id);
        handleOpenAiccForReference(source.id);
        message.info(inputs.length > 1 || sources.length > 1
            ? "存在多个输入，请明确添加移动云参考素材，并在参考栏手动移除不适用的输入；不会自动重试"
            : "请添加移动云参考素材，确认输入后手动生成；失败结果与原始错误会保留");
    }, [focusNode, handleOpenAiccForReference, handleReplaceMedia, message]);
            ...
            <section className="relative min-w-0 flex-1 overflow-hidden">
                ...
                >
                    ...
                    {visibleNodes.map((node) => (
                        <CanvasNode
                            ...
                            onReplaceAicc={handleRepairAiccReference}
                ...
                <CanvasToolbar
                    ...
                    onOpenVideoEditor={() => {
                        ...
                        const rawSelectedVideoNodes = Array.from(selectedNodeIds)
                            .map((id) => nodesRef.current.find((n) => n.id === id))
                            .filter((n): n is CanvasNodeData => Boolean(n && n.type === CanvasNodeType.Video && n.metadata?.content));

                        // 综合连线拓扑流向、镜头编号与多行故事板阅读流向，保证分镜时序绝对正确
                        const selectedVideoNodes = sortStoryboardVideoNodes(rawSelectedVideoNodes, connectionsRef.current);
                        if (selectedVideoNodes.length > 0) {
                            const ids = selectedVideoNodes.map((n) => n.id).join(",");
                            window.open(`/editor?projectId=${encodeURIComponent(projectId)}&batchNodes=${ids}`, "_blank");
                        } else {
                            window.open(`/editor?projectId=${encodeURIComponent(projectId)}`, "_blank");
                        }
                    }}
                />
                ...
                {contextMenu ? (
                    <CanvasNodeContextMenu
                        ...
                        onOpenVideoEditor={() => {
                            ...
                            setContextMenu(null);
                            window.open(`/editor?projectId=${encodeURIComponent(projectId)}&nodeId=${encodeURIComponent(contextMenu.nodeId)}`, "_blank");
                        }}
                ...
                {previewNode?.metadata?.content ? (() => {
                    ...
                    const isVideo = activeItemNode.type === CanvasNodeType.Video;
                    const previewSrc = isVideo
                        ? formatPlayableVideoUrl(activeItemNode.metadata?.content, activeItemNode.metadata?.model)
                        : activeItemNode.metadata?.content || "";
                    return (
                        <FullscreenPreview
                            src={previewSrc}
                            alt={activeItemNode.title || (isVideo ? "视频" : "图片")}
                            isVideo={isVideo}
...
// 替换媒体时统一清除上一份素材的身份、任务和批次状态，保留节点提示词与生成配置。
const REPLACED_MEDIA_METADATA: CanvasNodeMetadata = {
    content: undefined,
    storageKey: undefined,
    aiccUri: undefined,
    aiccChannelId: undefined,
    mimeType: undefined,
    bytes: undefined,
    ...
    klingImageNodeIds: undefined,
    klingMultiPrompt: undefined,
    klingElementList: undefined,
    mimoVoiceCloneAudioNodeId: undefined,
...
async function hydrateCanvasImages(nodes: CanvasNodeData[]) {
    return Promise.all(
        nodes.map(async (node) => {
            ...
            if (node.metadata?.aiccUri) return { ...node, metadata: { ...node.metadata, storageKey: undefined } };
            ...
            // 自动自愈：若历史视频节点保存的是未经转存的受保护端点，通过内部代理转存为本地媒体
            if (node.type === CanvasNodeType.Video && content && /\/videos\/[^/]+\/content(?:[?#]|$)/i.test(content)) {
                try {
                    const token = useUserStore.getState().token;
                    const taskIdMatch = content.match(/\/videos\/([^/?#]+)\/content/i);
                    const taskId = (taskIdMatch ? taskIdMatch[1] : "") || node.metadata?.videoTaskId || "";
                    const model = node.metadata?.model || "";
                    const fetchUrl = taskId
                        ? `/api/v1/videos/${encodeURIComponent(taskId)}/content?model=${encodeURIComponent(model)}`
                        : content;
                    ...
                    if (res.ok) {
                        ...
                        return { ...node, metadata: { ...node.metadata, content: media.url, storageKey: media.storageKey, videoTaskId: taskId || node.metadata?.videoTaskId } };
                    }
                } catch {}
            }
...
function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode): AiConfig {
    ...
    const chosenModel = mode === "text" ? resolveModelForCapability(config, node?.metadata?.model, "text") : node?.metadata?.model || defaultModel || (mode === "audio" ? defaultConfig.audioModel : config.model || defaultConfig.model);
