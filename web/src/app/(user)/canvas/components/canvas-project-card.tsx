...
export function CanvasProjectCard({ project }: { project: CanvasProject }) {
    ...
    const saveTitle = () => {
        renameProject(project.id, editingTitle.trim() || displayTitle);
        stopEditing();
    };
        ...
        <article className="group flex min-h-44 cursor-pointer flex-col justify-between rounded-2xl bg-[#f1eee8] p-5 transition hover:bg-[#ebe6dc] dark:bg-white/5 dark:hover:bg-white/10" onClick={() => !editing && open()}>
            ...
            <div className="mt-8 flex items-end justify-between gap-3">
                <p className="text-xs text-stone-500">
                    更新于 {Number.isNaN(Date.parse(project.updatedAt || "")) ? "刚刚" : new Date(project.updatedAt).toLocaleString("zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}
                </p>
                <div className="flex items-center gap-1" onClick={(event) => event.stopPropagation()}>
                    {editing ? (
                        ...
                        <>
                            <Button type="text" size="small" shape="circle" icon={<Download className="size-4" />} onClick={() => void exportCanvasProjects([project], displayTitle)} aria-label="导出" />
                            <Button type="text" size="small" shape="circle" icon={<Pencil className="size-4" />} onClick={() => startEditing(project.id, displayTitle)} aria-label="重命名" />
