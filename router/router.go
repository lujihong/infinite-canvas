...
func New() *gin.Engine {
	...
	api.GET("/auth/email/verification", gin.WrapF(handler.SendEmailVerification))
	api.POST("/auth/password/reset-request", gin.WrapF(handler.SendPasswordResetEmail))
	api.POST("/auth/password/reset", gin.WrapF(handler.ResetPassword))
	...
	v1.GET("/user/wallet", gin.WrapF(handler.UserWallet))
