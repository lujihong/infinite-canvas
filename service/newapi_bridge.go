...
func FetchUserConsumptionLogs(userID string) ([]ConsumptionLogItem, error) {
	...
	req, err := http.NewRequest(http.MethodGet, getNewAPIBaseURL()+"/api/log/token", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Authorization", "Bearer "+token)
	...
	logs := make([]ConsumptionLogItem, 0, len(res.Data))
	for _, l := range res.Data {
		// 中转站 500,000 quota 为基准单位，折合人民币与积分均按 10 倍换算校准
		moneyYuan := (float64(l.Quota) / float64(quotaPerUnit)) * 10.0
		// 工作台统一标准换算：1 元人民币 = 10 积分
		...
		if l.Other != "" {
			...
			if json.Unmarshal([]byte(l.Other), &otherData) == nil && otherData.TaskID != "" {
...
func FetchUserRechargeLogs(userID string) ([]RechargeLogItem, error) {
	...
	resp, err := client.Do(req)
	if err != nil {
		return nil, fmt.Errorf("请求充值记录失败: %w", err)
	}
	defer resp.Body.Close()
	...
	items := make([]RechargeLogItem, 0, len(res.Data.Items))
